/* bi-mem.c
 * last modified : 10/12/09 16:11:53 
 *
...
struct evtmem *newevt(char *data, int size, int efn, unsigned int evtn, int *flag){ 
  ...
  *flag = 0;
  ...
  if(!first){
    ...
    if(!(New->evtdata[efn] = (char*)malloc(size))){
	...
	if(evtn == p->evtn){
	  if(!p->evtdata[efn]){
	    ...
	    if(!(p->evtdata[efn] = (char*)malloc(size))){
...
struct nevtmem *newnevt(char *data, int size){
  ...
  New->next = NULL;
  New->nevtdata = NULL;
  if(!(New->nevtdata = (char*)malloc(size))){
 ...
 */
struct tsmem *newts(unsigned int evtn, long long int ts){
 ...
 */
long long int chkts(unsigned int evtn){

  if(!tsfirst){
    return -1;
  }else{
    //DB(printf("chkts %u : %u / %lld\n", evtn, tsfirst->evtn, tsfirst->ts));
    if(evtn == tsfirst->evtn){
      return tsfirst->ts;
    }else{
      return -1;
    }
...
int addrbuff(struct stbrstat *st, char *data, int sz){
  ...
  // for new rbuff
  trb = (stbrbuff*)malloc(sizeof(struct stbrbuff));
  trb->data = td;
  trb->blkn = st->blkn + 1;
  trb->next = NULL;
  trb->size = sz;
